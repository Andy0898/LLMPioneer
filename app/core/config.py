...
class AppConfig:
    ...
    def _get_config_directory(self) -> str:
        ...
        Default is LLMPioneer/config relative to the code directory.
        """
        # Check for environment variable first
        config_dir = os.getenv('PIONEER_CONFIG_DIR')
        if config_dir:
        ...
        if not config_dir:
            # Default: go up two levels from core directory to LLMPioneer, then to config
            current_dir = os.path.dirname(os.path.abspath(__file__))  # core directory (LLMPioneer/app/core)
            app_dir = os.path.dirname(current_dir)  # app directory (LLMPioneer/app)
            project_root = os.path.dirname(app_dir) # LLMPioneer directory
    ...
    def _get_base_output_directory(self) -> Optional[str]:
        ...
        """
        base_dir = os.getenv('PIONEER_OUTPUT_DIR')
    ...
    def load_pioneer_config(self, path: str = "config_pioneer.yaml"):
        ...
        try:
            ...
            # If config file doesn't exist, use defaults
            print(f"Warning: {path} not found. Using default LLMPioneer configuration.")
            data = {
                "sites": "",
                "data_folders": {
                    "json_data": "./data/json",
        ...
        # Convert relative paths to use PIONEER_OUTPUT_DIR if available
    ...
    def get_chatbot_instructions(self, instruction_type: str = "search_results") -> str:
        ...
        if (hasattr(self, 'pioneer') and 
            self.pioneer.chatbot_instructions and 
            instruction_type in self.pioneer.chatbot_instructions):
            return self.pioneer.chatbot_instructions[instruction_type]
    ...
    def get_headers(self) -> Dict[str, str]:
        ...
        if hasattr(self, 'pioneer') and self.pioneer.headers:
            return self.pioneer.headers
        return {}
    ...
    def get_allowed_sites(self) -> List[str]:
        """Get the list of allowed sites from Pioneer configuration."""
        return self.pioneer.sites if hasattr(self, 'pioneer') else []
    ...
    def is_tool_selection_enabled(self) -> bool:
        ...
        return self.pioneer.tool_selection_enabled if hasattr(self, 'pioneer') else True
    
    def is_memory_enabled(self) -> bool:
        """Check if memory functionality is enabled."""
        return self.pioneer.memory_enabled if hasattr(self, 'pioneer') else False
    
    def is_analyze_query_enabled(self) -> bool:
        """Check if query analysis is enabled."""
        return self.pioneer.analyze_query_enabled if hasattr(self, 'pioneer') else False
    
    def is_decontextualize_enabled(self) -> bool:
        """Check if decontextualization is enabled."""
        return self.pioneer.decontextualize_enabled if hasattr(self, 'pioneer') else True
    
    def is_required_info_enabled(self) -> bool:
        """Check if required info checking is enabled."""
        return self.pioneer.required_info_enabled if hasattr(self, 'pioneer') else True
    ...
    def load_sites_config(self, path: str = "sites.xml"):
        ...
        try:
            ...
            # Store the site configurations in pioneer config
            if hasattr(self, 'pioneer'):
                self.pioneer.site_configs = site_configs
    ...
    def get_site_config(self, site_name: str) -> Optional[SiteConfig]:
        ...
        if hasattr(self, 'pioneer') and self.pioneer.site_configs:
            return self.pioneer.site_configs.get(site_name)
        return None
            ...
            if self.conversation_storage_default in self.conversation_storage_endpoints:
                ...
                if self.conversation_storage_endpoints:
                    ...
                    self.conversation_storage = ConversationStorageConfig(
                        ...
                        collection_name="pioneer_conversations"
                    )
