...
class LoggingConfig:
    ...
    def _find_config_file(self, config_path: str) -> str:
        ...
        # Check for PIONEER_CONFIG_DIR environment variable
        ...
        # Go up to find the LLMPioneer root directory and then to config
        # logger -> misc -> python -> code -> PIONEER
        pioneer_root = current_dir
        for _ in range(4):  # Go up 4 levels
            pioneer_root = os.path.dirname(pioneer_root)
        
        config_dir = os.path.join(pioneer_root, 'config')
        full_path = os.path.join(config_dir, os.path.basename(config_path))
        if os.path.exists(full_path):
            return full_path
    ...
    def _ensure_log_directory(self):
        ...
        # Check for PIONEER_OUTPUT_DIR environment variable
        output_dir = os.getenv('PIONEER_OUTPUT_DIR')
        if output_dir:
    ...
    def get_logger(self, module_name: str) -> LoggerUtility:
        ...
        active_profile_name = os.getenv("PIONEER_LOGGING_PROFILE")
        profile_level = None
        ...
        # Get log file path - Use self.log_directory which respects PIONEER_OUTPUT_DIR
        log_file = None
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        profile = os.getenv("PIONEER_LOGGING_PROFILE", "development")
